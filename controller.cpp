...
int main()
{
    ...
    while (runloop)
    {
        ...
        { // initial neutral position FULLY ACTUATED
            ...
            Eigen::VectorXd current_q = robot->q();

             // Update the first six elements with q_initial.head(6)
             current_q.head(6) = q_initial.head(6);
             current_q.segment(6,12) = q_joints;
             joint_task->setGoalPosition(current_q);

                // Assign the updated state vector back to the robot
        ...
        { // crouching
            ...
            // if (abs(body_pos(2) - goalPos(2)) > 1e-2 && counter < 1)
            // {
            //     // if (abs(body_pos(2) - goalPos(2)) > 1e-2 )  { //body position
            //     body_task->setGoalPosition(body_pos - Vector3d(0,0,(body_pos(2)-goalPos(2))));//GOAL POSITION SETTING
        ...
        else if (state == LAUNCH)
        {
            ...
            // if (body_vel(2) >= 3.5)  { //3.7
                ...
                // joint_task->setGains(0, 40, 0);
                joint_task->setGains(400, 40, 0); // 18x1 vector for each joint DoF in RADIANS
                COM_task->setOriControlGains(800, 40, 0);
                // body_task->setOriControlGains(800,40,0);
                // cout << "updated gains" << "/n";
                // cout << robot->q();
                q_flight = robot->q();
            ...
            // body_task->setGoalOrientation(neutralOrientation);
            COM_task->setGoalPosition(comPos + Vector3d(0, 0, 0.2)); // GOAL POSITION SETTING needs tweaking
            COM_task->setGoalLinearVelocity(Vector3d(0, 0, 0.7));
            // cout << "jumping" << "\n";
