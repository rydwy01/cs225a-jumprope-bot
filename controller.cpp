...
int main()
{
    ...
    while (runloop)
    {
        ...
        // cout << time << "\n";
        // used to speed up or slow down jump rope (plus click 0 in between consecutive increase).  Additionally if it's "1" then that's the command to begin jumping
        // int state = redis_client.getInt(KEYBOARD_KEY); // STATE_KEY IN REDIS KEYS--string stateKey = whatever
        // update robot 1
        robot->setQ(redis_client.getEigen(JOINT_ANGLES_KEY));
        robot->setDq(redis_client.getEigen(JOINT_VELOCITIES_KEY));
        ...
        { // initial neutral position FULLY ACTUATED
            ...
            key_input = redis_client.getInt(KEYBOARD_KEY);
            // key_input = 1;
            cout << "posture" << endl;
            cout << "key input =" << key_input << endl;
            ...
            if (key_input == 1)
            {
                ...
                cout << "ready to change state" << endl;
                redis_client.setInt(KEYBOARD_KEY, 0);
                // cout << state << endl;
            }
            ...
            { // if error between joint angles and desired angles is <error
                ...
                // RYAN: implement logic here to decide when to begin jumping the first time
                // if (key_input == 1)
                // {
                //     state = FEET_FIXED_MOTION;
                //     cout << "ready to change state" << endl;
        ...
        else if (state == LAUNCH)
        {
            ...
            {
                cout << "made it to flight a second time" << endl;
        ...
        else if (state == FLIGHT)
        {
            ...
            if (forceFL(2) < -0.1 && forceFR(2) < -0.1 && forceRL(2) < -0.1 && forceRR(2) < -0.1)
            {
                state = FALL;
                // cout << "landing" << "\n";
                body_task->setPosControlGains(600, 185, 0); // before second gain was 200 for both body
                body_task->setOriControlGains(600, 185, 0);
                joint_task->setGains(350, 100, 0); // before was 300 in first gain
                COM_task->setPosControlGains(400, 40, 0);
                COM_task->setOriControlGains(400, 40, 0);
                q_land = robot->q();
                // counter++;
