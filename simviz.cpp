...
using namespace std;// mutex and globals
VectorXd ui_torques;
mutex mutex_torques, mutex_update;// specify urdf and robots
const string world_file = "./resources/world_spot.urdf";
const string robot_file = "./resources/spot.urdf";
const string robot_name = "spot";
const string camera_name = "camera_fixed";// dynamic objects information
...
void simulation(std::shared_ptr<Sai2Simulation::Sai2Simulation> sim);int main() {
    ...
    thread sim_thread(simulation, sim);    // while window is open:
...
void simulation(std::shared_ptr<Sai2Simulation::Sai2Simulation> sim) {
    ...
    Sai2Common::LoopTimer timer(sim_freq);    sim->setTimestep(1.0 / sim_freq);
    sim->enableGravityCompensation(true);
    sim->enableJointLimits(robot_name);    while (fSimulationRunning) {
